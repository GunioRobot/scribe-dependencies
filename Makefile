...
hadoop:
	cd hadoop-0.20.1+152/src/c++/libhdfs && JVM_ARCH=64 ac_cv_func_malloc_0_nonnull=yes sh configure --prefix=$(PREFIX) --with-java=$(JAVA_HOME) && make && make install && cp *.h $(PREFIX)/include && cd ..
