...
fb303: thrift
	cd thrift-0.2.0-rc0/contrib/fb303 && CPPFLAGS=-I$(shell pwd)/thrift-0.2.0-rc0/contrib/fb303/../../lib/cpp/src ./bootstrap.sh --with-thriftpath=$(shell pwd)/thrift-0.2.0-rc0/compiler/cpp && find . -name Makefile | xargs perl -pi -e 's/\$\(thrift_home\)\/bin/\$\(thrift_home\)/' && make
